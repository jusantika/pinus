...
import { MapPin, Calendar, Users, Search, Image as ImageIcon, BookHeart, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-100 pb-32">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8">
            ...
            <div className="relative group w-full lg:max-w-md flex-shrink-0">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3.5 bg-white/80 border border-gray-200/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] rounded-2xl text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all font-medium"
                value={search}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 relative">
        {loading ? (
          ...
          >
            <div className="w-20 h-20 bg-gray-900 text-white rounded-[1.25rem] shadow-xl rotate-3 flex items-center justify-center mx-auto mb-6">
              <BookHeart className="w-10 h-10 -rotate-3" />
            </div>
          ...
          <div className="relative pb-10">
            {/* The Central Line */}
            <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-1 bg-gray-200 transform md:-translate-x-1/2 rounded-full z-0"></div>

            <div className="flex flex-col gap-16 md:gap-24 relative z-10 pt-4">
              <AnimatePresence mode="popLayout">
                {displayedPlaces.map((place, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Milestone Node */}
                      <div className="absolute left-8 md:left-1/2 transform -translate-x-1/2 flex items-center justify-center w-12 h-12 rounded-full bg-gray-900 border-[4px] border-white shadow-[0_0_0_4px_rgba(243,244,246,1)] z-20">
                        <MapPin className="w-5 h-5 text-white" />
                      </div>

                      {/* Card Container */}
                      <div className={`w-full md:w-[45%] flex ${isLeft ? 'md:justify-end' : 'md:justify-start'} pl-20 md:pl-0 ${isLeft ? 'md:mr-auto' : 'md:ml-auto'}`}>
                        <div className="w-full group bg-white rounded-[2rem] shadow-[0_8px_20px_rgb(0,0,0,0.06)] border border-gray-200/60 overflow-hidden hover:shadow-[0_20px_40px_rgb(0,0,0,0.12)] transition-all duration-300 flex flex-col hover:-translate-y-1">
                          ...
                          <div className="p-6 flex flex-col">
                            <div className="flex justify-between items-start gap-4 mb-4">
                              <h3 className="text-xl font-extrabold text-gray-900 line-clamp-2 leading-tight">{place.name}</h3>
                              {(!place.photos || place.photos.length === 0) && (
                                <span className="shrink-0 px-3.5 py-1.5 bg-gray-100 text-gray-600 rounded-full text-[10px] font-extrabold uppercase tracking-wider mt-1">
                                  {place.category}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-col gap-3 text-sm text-gray-600 font-medium">
                              {place.visited_date && (
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <span>{new Date(place.visited_date).toLocaleDateString('id-ID', {
                              ...
                              {place.with_who && (
                                <div className="flex items-center gap-2.5">
                                  ...
                                  <span className="truncate">{place.with_who}</span>
                                </div>
                              )}
                              <div className="flex items-center gap-2.5">
